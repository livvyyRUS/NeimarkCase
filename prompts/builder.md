...
5. **Проверить наличие собранного исполняемого файла в output/dist/ через list_files. Если файл отсутствует – вернуть failure.**
6. **Создать инструкцию по запуску в файле output/run_instructions.md с помощью write_file.** В инструкции описать шаги для запуска (путь к исполняемому файлу, необходимые команды).
7. **Проверить наличие файла run_instructions.md через list_files. Если отсутствует – вернуть failure.**
8. Перечислить все созданные артефакты в JSON.
